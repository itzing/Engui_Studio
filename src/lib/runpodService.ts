    ...
    switch (modelId) {
      ...
      case 'krea2-turbo':
        const zImageInput: Record<string, any> = {
          ...
          ...(!input.media_inputs && input.init_image ? { init_image: input.init_image } : {}),
          ...(!input.media_inputs && input.source_image ? { source_image: input.source_image } : {}),
          ...(input._secure && { _secure: input._secure }),
          ...(input.media_inputs && { media_inputs: input.media_inputs }),
          ...(input.transport_request && { transport_request: input.transport_request }),
