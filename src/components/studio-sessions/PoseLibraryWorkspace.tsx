...
import { ArrowDown, ArrowUp, Copy, Download, Filter, ImageIcon, Plus, SlidersHorizontal, Upload } from 'lucide-react';
...
import type { StudioPoseCategorySummary, StudioPoseLibrarySettingsSummary, StudioPoseSummary } from '@/lib/studio-sessions/types';
...
function Toolbar({
  ...
  previewAgeDescription,
  onPreviewAgeDescriptionChange,
  onSaveSettings,
  settingsSaving,
  onExport,
  onOpenImport,
  ...
  onPreviewAgeDescriptionChange: (value: string) => void;
  onSaveSettings: () => void;
...
export default function PoseLibraryWorkspace({ route }: { route: PoseLibraryRoute }) {
  ...
  const [poseSettings, setPoseSettings] = useState<StudioPoseLibrarySettingsSummary | null>(null);
  const [previewAgeDescription, setPreviewAgeDescription] = useState('');
  ...
  const refreshPoseSettings = useCallback(async () => {
    if (!activeWorkspaceId) return;
    const data = await fetchJson(`/api/studio/pose-library/settings?workspaceId=${encodeURIComponent(activeWorkspaceId)}`, 'Failed to fetch pose library settings');
    const settings = data.settings as StudioPoseLibrarySettingsSummary | undefined;
    if (!settings) return;
    setPoseSettings(settings);
    setPreviewAgeDescription(settings.previewAgeDescription || '');
    setVariantCount(settings.defaultVariantCount || 4);
  }, [activeWorkspaceId, fetchJson]);
  ...
  }, [fetchJson, refreshCategories, refreshPoseSettings, refreshPoses, route]);
  ...
  async function savePoseSettings() {
    if (!activeWorkspaceId) return;
    const nextAge = previewAgeDescription.trim();
    if (poseSettings && poseSettings.previewAgeDescription === nextAge) return;
    setSettingsSaving(true); setError(null);
    try {
      const response = await fetch('/api/studio/pose-library/settings', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: activeWorkspaceId, previewAgeDescription: nextAge }) });
      const data = await response.json();
      if (!response.ok || !data?.success) throw new Error(data?.error || 'Failed to save pose library settings');
      setPoseSettings(data.settings ?? null);
      setPreviewAgeDescription(data.settings?.previewAgeDescription || '');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to save pose library settings');
    } finally {
