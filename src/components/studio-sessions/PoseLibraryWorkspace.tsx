...
export default function PoseLibraryWorkspace({ route }: { route: PoseLibraryRoute }) {
  ...
  async function moveCategory(categoryId: string, direction: -1 | 1) {
    ...
    setCategories(next);
    const response = await fetch('/api/studio/pose-library/categories/reorder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: activeWorkspaceId, ids: next.map((category) => category.id) }) });
    const data = await response.json();
    if (!response.ok || !data?.success) { setCategories(categories); setError(data?.error || 'Failed to reorder categories'); return; }
  ...
  async function movePose(poseId: string, direction: -1 | 1) {
    ...
    const targetIndex = index + direction;
    if (index < 0 || targetIndex < 0 || targetIndex >= sameCategory.length) return;
    const nextCategory = [...sameCategory];
    [nextCategory[index], nextCategory[targetIndex]] = [nextCategory[targetIndex], nextCategory[index]];
    setPoses((current) => current.map((item) => item.categoryId === pose.categoryId ? nextCategory.shift() ?? item : item));
    const response = await fetch('/api/studio/pose-library/poses/reorder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ workspaceId: activeWorkspaceId, categoryId: pose.categoryId, ids: sameCategory.map((item) => item.id).map((id) => id === poseId ? sameCategory[targetIndex].id : id === sameCategory[targetIndex].id ? poseId : id) }) });
    const data = await response.json();
