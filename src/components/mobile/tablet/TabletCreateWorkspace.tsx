    ...
    >
      ...
      <div className="flex h-full flex-col px-3 pb-3 pt-4">
        <div className="mb-2 flex items-center gap-2">
          ...
          <div className="text-xs text-muted-foreground">{totalCount}</div>
          <Button variant="ghost" size="icon" className="ml-auto h-7 w-7" onClick={onRefresh} aria-label="Refresh jobs" title="Refresh">
          ...
          <div className="flex h-full gap-2 pr-2">
            {isLoading ? (
              Array.from({ length: 8 }).map((_, index) => (
                <div key={index} className="aspect-square h-full max-h-[104px] shrink-0 rounded-md border border-border/70 bg-muted/20 animate-pulse" />
              ))
            ) : entries.length === 0 ? (
              ...
              entries.map(({ job }) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    className={cn(
                      'relative aspect-square h-full max-h-[104px] shrink-0 overflow-hidden rounded-md border bg-black transition-colors',
            ...
            {isLoadingMore ? (
              <div className="flex aspect-square h-full max-h-[104px] shrink-0 items-center justify-center rounded-md border border-border/70 bg-muted/20">
