    ...
    try {
        ...
        return NextResponse.json({
            ...
            job: {
                ...
                sceneSnapshotJson: (job as any).sceneSnapshotJson ?? null,
                sourcePromptDocumentId: (job as any).sourcePromptDocumentId ?? null,
                sourcePromptDocumentTitle: (job as any).sourcePromptDocumentTitle ?? null,
...
export async function DELETE(
    ...
    try {
        ...
        const status = message.includes('cancel it first') || message.includes('materialization finishes') ? 409 : 500;
        return NextResponse.json({
            success: false,
            error: message
        }, { status });
