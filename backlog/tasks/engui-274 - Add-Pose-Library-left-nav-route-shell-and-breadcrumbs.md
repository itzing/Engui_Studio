...
- [ ] Left nav includes `Pose Library` at the same level as `Portfolios`.
- [ ] `/studio-sessions/pose-library` renders the Pose Library home shell.
- [ ] Routes exist for all poses, category detail, and pose detail.
- [ ] Breadcrumbs match `Pose Library / All poses / Category / Pose` expectations and ancestors are clickable.
